...
class TestCase(unittest.TestCase):
    ...
    def test_getSubmission(self):
        ...
        res = user.functions.getSubmission(0,0).call()
        print("testing get admin submission")

        assert res == [60,70,65,60,60]

    def test_getSubmission2(self):
        #Set default account
        contract_address, abi = utils.deploy_n_transact(['../contracts/submission.sol'])
        w3.eth.defaultAccount = w3.eth.accounts[1]
        # Create the contract instance with the newly-deployed address
        ...
        assert res == [data_Accuracy,data_Precision,data_Recall,data_Fscore,60]

    def test_getSubmission3(self):
        #Set default account
        contract_address, abi = utils.deploy_n_transact(['../contracts/submission.sol'])
        w3.eth.defaultAccount = w3.eth.accounts[1]
        # Create the contract instance with the newly-deployed address
